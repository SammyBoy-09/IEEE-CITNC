...
import shriyaImg from '@/assets/team/aess/BUShriya.png';
import mohitImg from '@/assets/team/aess/Mohit.png';
import tanujaImg from '@/assets/team/aess/tanuja.png';
import rakshithaImg from '@/assets/team/aess/rakshitha.png';
import monishImg from '@/assets/team/aess/MonishR.png';
import aessLogo from '@/assets/team/aess/aess_logo.png';

// --- Background Component: Optimized CSS-based Stars ---
const StarField = () => {
  const stars = useMemo(() => {
  ...
  return (
    <div className="fixed inset-0 z-0 bg-slate-950 overflow-hidden will-change-auto">
      <style>{`
        @keyframes twinkle {
        ...
        <div className="flex items-center justify-between h-16">
          ...
          >
            <a 
              ...
              className="flex items-center gap-2 sm:gap-3 text-base sm:text-lg md:text-xl font-bold text-blue-400 hover:text-blue-300 transition-colors"
            >
              <img src={aessLogo} alt="AESS Logo" className="h-10 w-10 sm:h-24 sm:w-24 object-contain" />
              <span className="hidden sm:inline">IEEE AESS CITNC</span>
              <span className="sm:hidden">AESS CITNC</span>
            </a>
          </motion.div>
          ...
          <div className="hidden lg:flex items-center space-x-6">
            ...
            <motion.button
              onClick={() => navigate('/')}
              className="flex items-center gap-2 text-slate-300 hover:text-blue-400 transition-colors ml-4 px-3 py-1.5 border border-slate-700 hover:border-blue-500/50 rounded-lg focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
          ...
          <div className="lg:hidden flex items-center gap-3">
            <button
              onClick={() => navigate('/')}
              className="flex items-center gap-1 text-slate-300 hover:text-blue-400 transition-colors px-2 py-1.5 border border-slate-700 rounded-lg focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
              aria-label="Back to home"
            >
              <ArrowLeft className="h-4 w-4" />
              <span className="text-sm font-medium">Back</span>
...
const PillarCard: React.FC<PillarCardProps> = ({ icon, title, description, index = 0 }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, margin: "-50px" }}
    transition={{ duration: 0.5, delay: index * 0.1 }}
    className="bg-slate-900/80 backdrop-blur-sm p-6 rounded-2xl border border-slate-800 hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.3)] hover:-translate-y-1 transition-all duration-300"
  >
    <div
      className="text-5xl mb-4 inline-block hover:scale-110 transition-transform duration-300"
      role="img"
      aria-label={title}
    >
      {icon}
...
const EventCard: React.FC<EventCardProps> = ({ type, title, description, badge, badgeColor, index = 0 }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, margin: "-50px" }}
    transition={{ duration: 0.5, delay: index * 0.1 }}
    className="bg-slate-900/80 backdrop-blur-sm p-8 rounded-2xl border border-slate-800 hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.3)] hover:-translate-y-1 transition-all duration-300 h-full flex flex-col"
  >
    <div className="flex items-start justify-between mb-5">
      <span className="text-sm text-slate-400 uppercase tracking-wider">{type}</span>
      <span className={`${badgeColor} text-white text-sm px-4 py-1.5 rounded-full font-semibold`}>
        {badge}
      </span>
    </div>
    <h3 className="text-2xl font-bold mb-3 text-blue-400">{title}</h3>
    <p className="text-slate-300 text-base leading-relaxed mb-6 flex-grow">{description}</p>
    <div
      className="block w-full bg-slate-700 text-slate-400 text-center py-3 px-4 rounded-lg font-semibold text-base cursor-not-allowed mt-auto"
    >
      Coming Soon
    </div>
  </motion.div>
...
const ResourceLinkCard: React.FC<ResourceLinkCardProps> = ({ icon, title, description, url, index = 0 }) => (
  <motion.a
    href={url}
    ...
    viewport={{ once: true, margin: "-50px" }}
    transition={{ duration: 0.5, delay: index * 0.1 }}
    className="bg-slate-900/80 backdrop-blur-sm p-6 rounded-2xl border border-slate-800 hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.3)] hover:-translate-y-1 transition-all duration-300 block"
  >
    <div className="text-4xl mb-4" role="img" aria-label={title}>{icon}</div>
    <h3 className="text-lg font-bold mb-3 text-blue-400">{title}</h3>
    <p className="text-slate-300 text-sm leading-relaxed">{description}</p>
...
const ProjectCard: React.FC<ProjectCardProps> = ({ icon, title, description, status, index = 0 }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, margin: "-50px" }}
  ...
  return (
    <div className="relative min-h-screen text-white font-sans selection:bg-blue-500/30">
      ...
      <header id="home" className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-6 overflow-hidden pt-16">
        ...
        >
          <h1 className="text-4xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black mb-4 sm:mb-6 bg-clip-text text-transparent bg-gradient-to-b from-white to-blue-400 pb-2 sm:pb-4">
            IEEE Aerospace and Electronic Systems Society
          </h1>
          <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-slate-300 mb-6 sm:mb-8 md:mb-10 max-w-3xl mx-auto leading-relaxed px-4">
            Empowering students to lead in Aviation, Space, and Electronic Systems. 
            <br />
            <span className="text-blue-400 font-semibold">Join the Cambridge Institute of Technology North Campus <br className="hidden sm:inline" />AESS Student Branch Chapter</span>
          </p>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <div className="space-y-6 text-slate-300 text-lg leading-relaxed">
              <p>
                Welcome to the <span className="text-blue-400 font-semibold">IEEE Aerospace and Electronic Systems Society (AESS)</span> Chapter at Cambridge Institute of Technology North Campus, Kundana, Bangalore.
              </p>
              <p>
      ...
      <section id="pillars" className="relative z-10 py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-slate-900/50 backdrop-blur-sm">
      ...
      {/* 4. EVENTS SECTION */}
      <section id="events" className="relative z-10 py-16 sm:py-20 md:py-24 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
      ...
      <section id="dlp" className="relative z-10 py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-slate-900/50 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
      ...
      <section id="benefits" className="relative z-10 py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-slate-900/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto">
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
      ...
      {/* 7. RESOURCES SECTION */}
      <section id="resources" className="relative z-10 py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-slate-900/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto">
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8 text-center">
            ...
            <div>
              ...
              <ul className="space-y-2 text-slate-400 text-sm">
                ...
                <li>
                  <button onClick={() => navigate('/guidelines')} className="hover:text-blue-400 transition-colors">
                    IEEE Guidelines
                  </button>
                </li>
              </ul>
